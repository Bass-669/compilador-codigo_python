...
def main():
    if len(sys.argv) < 2:
        print("Uso: script.py <carpeta_con_html>")
        sys.exit(1)
    ...
    for html_file in html_files:
        nombre_salida = os.path.splitext(html_file)[0] + '.txt'
        ruta_salida = os.path.join(carpeta_reportes, nombre_salida)
        
        # Si el archivo de salida ya existe, omitimos
        if os.path.exists(ruta_salida):
            print(f"Archivo de salida ya existe, omitiendo: {nombre_salida}")
            continue
        
        ruta_html = os.path.join(carpeta_reportes, html_file)
        
        if process_html_file(ruta_html, ruta_salida):
            print(f"Procesado: {html_file}")
            archivos_procesados += 1
        else:
            archivos_sin_tabla += 1
    
    print(f"\nProcesamiento completado. Archivos procesados correctamente: {archivos_procesados}")
    print(f"Archivos sin tabla de Diámetro: {archivos_sin_tabla}")
