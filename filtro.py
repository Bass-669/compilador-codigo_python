...
def format_data_row(row):
    cells = row.find_all(['td', 'th'], class_=['RWReport', 'RWReportSUM'])
    if not cells or len(cells) < 5:
        return None
    ...
    cell_texts = []
    for cell in cells:
        # Manejar tablas anidadas en la columna de distribución
        if cell.find('table'):
            table = cell.find('table')
            # Extraer solo el valor numérico de la tabla
            dist_text = table.find_all('td')[-1].get_text(strip=True)
            cell_texts.append(dist_text.split('&nbsp;')[0].strip())
        else:
            text = cell.get_text(strip=True)
    ...
    # Construir primera línea: primeras 4 celdas con 2 espacios antes del porcentaje
    first_line = ' '.join(cell_texts[:3])
    if len(cell_texts) > 3:
        first_line += '  ' + cell_texts[3]  # Dos espacios antes del porcentaje
    
    # Construir segunda línea: resto de celdas
    second_line = ' '.join(cell_texts[4:])
...
def process_html_file(html_file, output_file):
    ...
    # Procesar todas las filas excepto la primera (cabecera) y la última (total general)
