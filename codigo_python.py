...
def procesar_datos(entrada, torno, mes, dia, anio):
    ...
    for b in extraer_bloques(entrada):
        ...
        txt = sub[0] if not re.match(r'^\d', sub[0]) else ""
        datos = sub[1:] if txt else sub
        if not txt and len(sub) > 0:
            txt = sub[0]
        bloque_texto = " ".join(b).upper()
        tipo_bloque = "PODADO" if "PODADO" in bloque_texto else "REGULAR"
        valor_d_f_fin = hoja.cell(row=f_fin, column=4).value
        try:
            if valor_d_f_fin is not None:
                valor_d_f_fin = float(str(valor_d_f_fin).replace(",", "."))
        except:
            pass
    ...
    try:
        ...
        if not hoja_nueva_existia:
            ...
            if not hoja_anterior:
                return messagebox.showwarning("Orden inválido", f"No se encontró hoja anterior para insertar '{nueva}'.")
    ...
    try:
        ...
        try:
            ...
            for chart_obj in sheet_excel.ChartObjects():
                try:
                    chart = chart_obj.Chart # Obtener el eje de categorías (eje X)   
                    x_axis = chart.Axes(1)
...
def escribir_valor_bloque(hoja, col_dia, torno, valor, tipo_bloque):
    ...
    # except Exception as e:
    #     messagebox.showerror("Error", f"Fallo al escribir {tipo_bloque}:\n{e}")
